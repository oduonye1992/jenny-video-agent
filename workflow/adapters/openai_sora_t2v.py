...
Uses the OpenAI SDK directly (not FAL) for text-to-video generation.
No start frame required — pure text-to-video.
...
@register("openai_sora_t2v")
class OpenAISoraT2VAdapter(Adapter):
    """Wraps OpenAI Sora 2 for text-to-video generation (no start frame).
    ...
    def submit(self, config: dict) -> str:
        ...
        print(f"  [openai_sora_t2v] Submitted video: {video_id}", file=sys.stderr)
        return video_id
